...
> **v2.1 更新**: 架构重构 - 消除硬编码ID，实现能力包配置动态派生 (2026-01-01)
...
## 2.5 架构升级 (v2.1): 配置动态派生

为了提高扩展性和降低维护成本，v2.1 引入了"能力包配置自动派生"机制 (Phase 3)。

### 2.5.1 核心变更
不再在 `analysisPackages.ts` 中手动维护 Prompt 列表及其 Python 依赖。而是将配置下沉到每个 Prompt 文件中：

```typescript
// UserPrompt 接口新增字段
export interface UserPrompt {
    id: string;
    slug: string;           // 语义化引用 (如 'worker-distribution-v1')
    packageId: string;      // 所属能力包 (如 'basic', 'sklearn')
    requiredPackages: string[]; // 依赖 (如 ['pandas', 'numpy'])
    outputCharts: string[]; // 输出图表类型
    // ...
}
```

### 2.5.2 自动派生流程
1. **Registry 扫描**: `generateAnalysisPackages()` 函数扫描 `PromptRegistry` 中所有已注册的 L2 Prompt。
2. **分组汇总**: 按 `packageId` 分组。
3. **依赖推断**: 自动收敛每个包的 `requiredPackages` 并去重，生成最终的 Pyodide 加载列表。
4. **服务适配**: 业务代码（L1 Validator/Router）不再使用字符串字面量 ID，而是引用 `PROMPT_IDS` 常量。
...
### 10.0 MVP 应急修复（P0 - 2026-01-01 补充）

> **背景**: 2026-01-01 端到端测试发现 AI 生成代码质量问题（5个洞察中4个失败），根本原因是 `TEMPLATE_FILL` 模式过于简单。
> **决策**: MVP 阶段执行方案 A+B 作为兜底，P1 阶段开放用户编辑前完成完整校验体系。
...
**修复问题**:
- ✅ 数组参数未加引号 → `NameError: name 'median_income' is not defined`
- ✅ 字符串参数在 Python 中需要引号

#### 方案 B: 列名校验层（立即修复）

**位置**: `src/hooks/useInsightLoaderV2.ts` (执行前校验)

```typescript
// 执行前校验列名是否存在
const validColumns = 有效列名; // 从 DuckDB DESCRIBE 获取
const invalidParams = Object.entries(suggestion.params).filter(([key, val]) => {
    if (key.includes('col') || key === 'column_name') {
        return !validColumns.includes(String(val));
    }
    return false;
});

if (invalidParams.length > 0) {
    logger.warn('AI洞察', '检测到无效列名，跳过执行', { invalidParams });
    continue; // 跳过此洞察
}
```

**修复问题**:
- ✅ 列名不存在导致空数据绘图 → `IndexError: index 0 is out of bounds`
- ✅ 防止执行错误代码
...
| 错误类型     | 原因                    | 影响      | 修复方式          |
| ------------ | ----------------------- | --------- | ----------------- |
| `IndexError` | 列名不存在 → 空数据绘图 | 洞察3失败 | 方案B：执行前校验 |
| `NameError`  | 数组参数未加引号        | 洞察5失败 | 方案A：类型转换   |

**详细分析**: 参见 [AI代码质量问题分析报告](../../05年-项目管理/51-管理-错误日志记录.md#2026-01-01-端到端测试报错分析)

---

### 10.1 四层质量把控体系（P1 阶段实施）
