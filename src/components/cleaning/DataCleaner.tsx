    ...
    useEffect(() => {
        ...
        const loadStats = async () => {
            try {
                ...
                console.error(t('cleaning.loadStatsFailed'), e);
            }
        };
        loadStats();
        return () => { active = false; };
                        ...
                        <div key={col.name} className="columnStatCard">
                            ...
                            <div className="statBar">
                                <div
                                    ...
                                    title={t('cleaning.nullsTooltip', { percent: ((col.nullCount / col.total) * 100).toFixed(1) })}
                                />
                            </div>
                            <div className="statMeta">
                                {col.min !== null && <span>{t('cleaning.min')}: {Number(col.min).toFixed(1)}</span>}
                                {col.uniqueCount > 0 && <span>{t('cleaning.uniq')}: {col.uniqueCount}</span>}
                            </div>
                        </div>
                    ))}
                </div>
