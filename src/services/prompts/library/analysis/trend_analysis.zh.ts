...
/**
 * 生成趋势分析的 prompt（内部实现 - 中文）
 * @param 时间序列数据 - 按时间排序的异常数统计数据，数组格式：[{ date: string; count: number }, ...]
 * @returns prompt 字符串
 */
export function generateTrendPromptInternal(时间序列数据: { date: string; count: number }[]): string {
    // 将数据转成易读的文本描述
...
/**
 * 解析 AI 返回的趋势分析结果（内部实现 - 中文）
 * @param AI返回结果 - Gemini API 返回的原始文本
 * @returns 一句话趋势总结字符串（若解析失败返回默认兜底文案）
 */
export function parseTrendResultInternal(AI返回结果: string): string {
