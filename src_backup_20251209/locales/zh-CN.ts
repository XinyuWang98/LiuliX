...
export const zhCN: LanguageConfig = {
    ...
    translations: {
        ...
        settings: {
            ...
            // DuckDB Errors
            dbNotReady: '数据库未初始化',
            opfsFailed: 'OPFS 挂载失败，降级为内存模式',
            opfsSuccess: '🦆 DuckDB 引擎已初始化',
            parseSuccess: '解析完成，耗时 {time}ms',
        ...
        cleaning: {
            title: '数据清洗 (DuckDB Native)',
            currentTable: '当前表',
            dedup: '自动去重',
            fillNull: '填补缺失',
            normalize: '列名标准化',
            dropEmpty: '删除空行',
            sqlPlaceholder: '生成的 SQL 会显示在这里...',
            runSQL: '执行 SQL',
            intentDedup: '删除重复行',
            intentFillNull: '将数字列的 NULL 填为 0，文本列填为 "Unknown"',
            intentNormalize: '将所有列名转为 snake_case (小写 + 下划线)',
            intentDropEmpty: '删除所有字段都为空的行',
        },
